...
def HistChart(data,colnum):
    ...
    fig = px.histogram(data, x = colnum,template='plotly_dark', title=title_plot, labels={colnum:colnum})
...
def PieChart(data,colcat):
    ...
    fig = px.pie(values = values, names = labels,template='plotly_dark', title="Pie Chart of "+ colcat)
    return fig

#Numerica
def ViolinChartsecond(data,colcat,colnum):
    fig = px.violin(data, y=colnum, x=colcat, box=True, color = colcat, points="all",template='plotly_dark', title = "Violin Plot between "+ colcat + " and "+ colnum)
    fig.update_xaxes(tickangle=70)
    return fig
...
def LineChart(data,colDate,colnum):
    ...
    fig = px.line(data, x=year_name, y=colnum,template='plotly_dark',title=title)
    return fig
